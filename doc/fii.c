...
/*
	...
	* Ceci va au-delà de la portée de cet article, et en général il suffira d’indiquer NULL ici.
	...
	* start_routine du thread. Si l’on souhaite passer plusieurs paramètres à cette fonction,
	* il n’y a pas d’autre choix que de lui renseigner ici un pointeur vers une structure de données.
*/
int	pthread_create(pthread_t *restrict_thread,
	...
	void *restrict arg);
/*
	*thread : l’identifiant du thread qu’on attend. Le thread spécifié ici doit être joignable
	* (c’est à dire non détaché - voir ci-dessous).
	*retval : un pointeur vers une variable qui peut contenir la valeur de retour de la fonction routine du thread 
	* la fonction start_routine qu’on a fournie lors de la création du thread). Si on n’a pas besoin de cette valeur,
